...
import type { Task, Goal, Doc } from '@/lib/types'
import {
...
function TaskRow({ t, today, uid }: { t: Doc<Task>; today: string; uid: string }) {
  ...
  return (
    <div className="group flex items-center gap-3 rounded-xl px-2 py-2 transition-colors hover:bg-cream/60">
      ...
      {t.due && (
        <span className={cx('text-xs tabular-nums', overdue ? 'font-medium text-rose' : 'text-mist')}>
...
function Tasks() {
  ...
  /** Calendar sync: open tasks with due dates + goals with target dates. */
    ...
    for (const t of tasks) {
      if (!t.done && t.due) {
        events.push({
          ...
          title: `☐ ${t.title}`,
          date: t.due,
          description: `Tend task${t.priority === 2 ? ' · high priority' : ''}`,
        })
      }
    }
      ...
      <PageHeader
        ...
        action={
          <div className="flex flex-col items-end gap-1">
            ...
            <p className="text-[11px] text-mist">Open the .ics file on your phone → add to Apple/Google Calendar</p>
          </div>
        }
      />

      <Card className="mb-5">
        <form onSubmit={addTask} className="grid gap-3 md:grid-cols-[1fr_10rem_9rem_auto] md:items-end">
          <Field label="Task">
